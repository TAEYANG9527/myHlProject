...
import com.alibaba.wukong.im.Message.MessageStatus;
import com.alibaba.wukong.im.MessageBuilder;
import com.alibaba.wukong.im.MessageContent.MediaContent;
import com.alibaba.wukong.im.MessageContent.MessageContentType;
    ...
    private MessageListener mMessageListener = new MessageListener() {
        ...
        public void onChanged(List<Message> list) {
            for (Message message : list) {
                ...
                if ((MessageContentType.IMAGE == type || MessageContentType.AUDIO == type)
    ...
    public void sendAlbumImage(final String url, final Conversation conversation, final boolean isNeedCompress) {
        ...
        thread.start(new Runnable() {
            ...
            public void run() {
                ...
                if (TextUtils.isEmpty(imgPath)) {
                    Toast.makeText(mContext,mContext.getString(R.string.im_error_upload),Toast.LENGTH_SHORT).show();
//                    ToastUtil.showToast(mContext, mContext.getString(R.string.im_error_upload));
                    return;
                }
                //创建图片消息并发送
                ...
                message.sendTo(conversation, new Callback<Message>() {
                    ...
                    public void onSuccess(Message message) {
                        ...
                        ConversationListUtil.updateGroupMessageExtension(message, RenheApplication.getInstance().getUserInfo().getName(),
                                RenheApplication.getInstance().getUserInfo().getUserface());
                        EventBus.getDefault().post(new SendMsgSuccessEvent(message));
                        if (isNeedCompress) {
                            //消息的图片上传成功后,存缓存
                            if (mImageCache != null) {
                                String remoteUrl = ((MediaContent) message.messageContent()).url();
