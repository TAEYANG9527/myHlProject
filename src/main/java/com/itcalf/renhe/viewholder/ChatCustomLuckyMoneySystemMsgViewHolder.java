...
public class ChatCustomLuckyMoneySystemMsgViewHolder extends RecyclerHolder {
    ...
    public ChatCustomLuckyMoneySystemMsgViewHolder(Context context, View itemView, RecyclerView chatRecyclerView,
    ...
    public void initView(RecyclerHolder holder, Object messageObj, final int position) {
        ...
        if (!TextUtils.isEmpty(highlight) && msg.contains(highlight)) {
            ...
            contentSpannableString.setSpan(new ForegroundColorSpan(context.getResources()
                            .getColor(R.color.luckymoney_chat_rob_sysmsg_color)), index, index + highlight.length(),
                    Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            contentSpannableString.setSpan(new ChatUtils.SystemMsgSpanClick(context, luckySid, 2), index,
