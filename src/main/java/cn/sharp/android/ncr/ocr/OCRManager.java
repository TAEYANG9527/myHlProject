...
public class OCRManager {
	 ...
	 */
	public native int[] native_ncr(NativeOCRItems items, byte[] pgmContent, int width, int height);

	public native int[] native_ncr_from_jpeg(NativeOCRItems items, byte[] jpegData);
	...
	private OCRItems doRec(PgmImage pgmImage) {
		...
		int[] fieldLength = native_ncr(nativeOCRItems, pgmImage.getContent(), pgmImage.getWidth(), pgmImage.getHeight());
		if (fieldLength == null) {
			Log.e(TAG, "doRec pgm failure");
			return null;
		} else {
			Log.i(TAG, "doRec pgm success");
	...
	public OCRItems rec(byte[] jpeg) {
		...
		Log.i(TAG, "execute doRec method");
		return doRec(jpeg);
	}
	...
	private OCRItems doRec(byte[] jpeg) {
		NativeOCRItems nativeOcrItems = new NativeOCRItems();
		try {
			Log.i(TAG, String.valueOf(jpeg.length));
			...
			Log.e(TAG, "rec failure", e);
			return null;
		}
	}

	public OCRItems recPgm(byte[] jpeg) {
		if (jpeg == null) {
			return null;
		}
		return doRec(jpeg);
	...
	private OCRItems doRecPgm(byte[] jpeg) {
		NativeOCRItems nativeOcrItems = new NativeOCRItems();
		try {
			int[] fieldLength = native_ncr(nativeOcrItems, jpeg, 1600, 1200);
			if (fieldLength == null || fieldLength.length <= 0) {
				return null;
			} else {
				Log.d(TAG, "rec pgm success");
				OCRItems ocrItems = new OCRItems(nativeOcrItems, fieldLength);
				return ocrItems;
			}
		} catch (Exception e) {
			Log.e(TAG, "rec pgm failure", e);
			return null;
		}
	}
