...
public class AisenChatTextView extends TextView {
    ...
    private long openId;//IM urserID

    public AisenChatTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    public AisenChatTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public int getEmojiSize() {
        if (null != BaseActivity.getRunningActivity())
            return BaseActivity.getRunningActivity().getResources().getDimensionPixelSize(R.dimen.S4);
        else
            return RenheApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.S4);
    }
    ...
    public void setWebLinkColor(String webLinkColor) {
    ...
    public void setContent(String text) {
        ...
        SpannableString spannableString = stringMemoryCache.get(key + openId);
    ...
    class EmotionTask extends WorkTask<Void, SpannableString, Boolean> {
        ...
        public Boolean workInBackground(Void... params) throws TaskException {
            ...
            while (localMatcher.find()) {
                ...
                if (data == null)
                    continue;
                MyBitmap mb = BitmapLoader.getInstance().getImageCache().getChatBitmapFromMemCache(key, null);
                Bitmap b = null;
                if (mb != null) {
                    b = mb.getBitmap();
                } else {
...
//                    int size = BaseActivity.getRunningActivity().getResources().getDimensionPixelSize(R.dimen.emotion_size);
                    ...
                    // 添加到内存中
                    BitmapLoader.getInstance().getImageCache().addChatBitmapToMemCache(key, null, new MyBitmap(b, key));
                }
...
//            Linkify.addLinks(spannableString, dd, scheme);
            ...
            for (URLSpan urlSpan : urlSpans) {
                weiboSpan = new CustomURLSpan(urlSpan.getURL(), atMemmbers, openId == RenheApplication.getInstance().getUserInfo().getImId());
                if (TextUtils.isEmpty(webLinkColor))
                    weiboSpan.setColor(Color.parseColor("#4492EC"));
                else
                    weiboSpan.setColor(Color.parseColor(webLinkColor));
            ...
            stringMemoryCache.put(key + openId, spannableString);
            if (Constants.DEBUG_MODE)
