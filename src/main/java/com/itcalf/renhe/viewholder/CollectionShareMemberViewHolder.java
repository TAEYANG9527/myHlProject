...
/**
 * 收藏的“分享的链接”
 * Created by wangning on 2016/3/18.
 */
public class CollectionShareMemberViewHolder extends RecyclerHolder {
    ...
    private MyCollection.MessageBoardShareInfo messageBoardShareInfo;
    //item 头部布局
    ...
    public TextView forwardJobTv;
    public RelativeLayout forwardRl;
    private ImageView forwardIv;
    public DateUtil dateUtil;

    public CollectionShareMemberViewHolder(Context context, View itemView, RecyclerView collectionRecyclerView, RecyclerView.Adapter adapter) {
        ...
        forwardIv = (ImageView) itemView.findViewById(R.id.forward_archive_pic_iv);
        forwardRl = (RelativeLayout) itemView.findViewById(R.id.forward_rl);
        contentTv = (AisenTextView) itemView.findViewById(R.id.content_tv);
        forwardNameTv = (TextView) itemView.findViewById(R.id.forward_archive_name_tv);
        forwardJobTv = (TextView) itemView.findViewById(R.id.forward_archive_job_tv);
    ...
    public void initView(RecyclerHolder holder, Object mCollectionInfo, int position) {
        ...
        if (null == messageBoardInfo)
            return;
        messageBoardShareInfo = collectionInfo.getMessageBoardShareInfo();
        if (null == messageBoardShareInfo)
            return;
        initSelfInfoView();
        String content = messageBoardInfo.getContent().trim();//正文内容
        String shareName = messageBoardShareInfo.getShareName();//分享的人脉的姓名
        String shareJob = messageBoardShareInfo.getShareJob();//分享的人脉的工作
        String shareCompany = messageBoardShareInfo.getShareCompany();//分享的人脉的公司
        ...
        if (!TextUtils.isEmpty(shareName)) {
            forwardNameTv.setVisibility(View.VISIBLE);
            forwardNameTv.setText(shareName);
        } else {
            forwardNameTv.setVisibility(View.GONE);
        }

        if (!TextUtils.isEmpty(shareJob) && !TextUtils.isEmpty(shareCompany)) {
            forwardJobTv.setVisibility(View.VISIBLE);
            forwardJobTv.setText(shareJob + "\n" + shareCompany);
        } else if (!TextUtils.isEmpty(shareJob) && TextUtils.isEmpty(shareCompany)) {
            forwardJobTv.setVisibility(View.VISIBLE);
            forwardJobTv.setText(shareJob);
        } else if (TextUtils.isEmpty(shareJob) && !TextUtils.isEmpty(shareCompany)) {
            forwardJobTv.setVisibility(View.VISIBLE);
            forwardJobTv.setText(shareCompany);
        } else {
            forwardJobTv.setVisibility(View.GONE);
        }
        String imageUrl = messageBoardShareInfo.getSharePicUrl();
        if (!TextUtils.isEmpty(imageUrl)) {
            imageLoader.displayImage(imageUrl, forwardIv, CacheManager.circleImageOptions);
        }
        initListener();
    }
