...
public class AddEduInfoTask extends BaseAsyncTask<MessageBoardOperationWithErroInfo> {
	...
	protected MessageBoardOperationWithErroInfo doInBackground(String... params) {
		...
		reqParams.put("notes", params[9]);//
		reqParams.put("schoolName", params[10]);//
		try {
			MessageBoardOperationWithErroInfo mb = (MessageBoardOperationWithErroInfo) HttpUtil
					.doHttpRequest(Constants.Http.ADD_EDU_INFO, reqParams, MessageBoardOperationWithErroInfo.class, null);
			return mb;
		} catch (Exception e) {
			e.printStackTrace();
		}
