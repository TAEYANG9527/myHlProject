	...
	private void init(Context ctx) {
		...
		this.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				if (event.getX() > et.getWidth() - et.getPaddingRight() - imgX.getIntrinsicWidth() - 20) {
					et.setText("");
		...
		this.setOnFocusChangeListener(new OnFocusChangeListener() {
			...
			public void onFocusChange(View arg0, boolean arg1) {
				...
				if (arg1) {
					if (editText.getText().toString().trim().length() > 0) {
						if (isSearch) {
							editText.setCompoundDrawablesWithIntrinsicBounds(
									getResources().getDrawable(R.drawable.edittext_search), null,
					...
					if (isSearch) {
						editText.setCompoundDrawablesWithIntrinsicBounds(getResources().getDrawable(R.drawable.edittext_search),
								null, null, null);
					} else {
