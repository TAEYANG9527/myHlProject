 ...
 */
public class ClipAdLogoImageView extends ImageView implements View.OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener {

    private static final int BORDERDISTANCE = ClipView.BORDERDISTANCE;
    private int IMAGE_WIDTH = 650;
    ...
    public ClipAdLogoImageView(Context context) {
        this(context, null);
        this.mContext = context;
        initImageWidth(context);
    }
    ...
    public ClipAdLogoImageView(Context context, AttributeSet attr) {
        this(context, attr, 0);
        this.mContext = context;
        initImageWidth(context);
    }

    public ClipAdLogoImageView(Context context, AttributeSet attr, int defStyle) {
    ...
    private void initImageWidth(Context context) {
        IMAGE_WIDTH = (int) DensityUtil.getMetricsWidth(context) - 100;
    }
    ...
    private void configPosition() {
        ...
        Float hf = (float) mFocusWidth * AdLogoClipView.heightScale;
        mFocusHeight = hf.intValue();
        float yScale = (float) mFocusHeight / bitmap.getHeight();
        float xScale = (float) mFocusWidth / bitmap.getWidth();
        //��ʼ����С���ŷ�Χ
        ...
        if (drawableWidth <= drawableHeight) {
            // �ж�ͼƬ����Ƿ�С�ڱ߿���, ��������ü��߿�
            if (drawableWidth < mFocusWidth) {
                baseMatrix.reset();
                scale = (float) mFocusWidth / drawableWidth;
                //ͼƬ��С������С���ű���
                minScale = 1;
                // ����
            ...
            //�ж�ͼƬ�߶��Ƿ�С�ڱ߿�߶ȣ����С��������������п�
            if (drawableHeight < mFocusHeight) {

                baseMatrix.reset();
                scale = (float) mFocusHeight / drawableHeight;
                //ͼƬ��С������С���ű���
                minScale = 1;
                baseMatrix.postScale(scale, scale);
            }
    ...
    private class AnimatedZoomRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (((deltaScale > 1f) && (currentScale < targetZoom)) || ((deltaScale < 1f) && (targetZoom < currentScale))) {
                ...
                // again
                postOnAnimation(ClipAdLogoImageView.this, this);
    ...
    private void checkMatrixBounds() {
        ...
        final float viewHeight = getHeight();
        // �ж��ƶ������ź�ͼƬ��ʾ�Ƿ񳬳��ü���߽�
        if (rect.top > (viewHeight - mFocusHeight) / 2) {
            deltaY = (viewHeight - mFocusHeight) / 2 - rect.top;
        }
        if (rect.bottom < (viewHeight + mFocusHeight) / 2) {
            deltaY = (viewHeight + mFocusHeight) / 2 - rect.bottom;
        }
        if (rect.left > (viewWidth - mFocusWidth) / 2) {
            deltaX = (viewWidth - mFocusWidth) / 2 - rect.left;
        }
        if (rect.right < (viewWidth + mFocusWidth) / 2) {
            deltaX = (viewWidth + mFocusWidth) / 2 - rect.right;
        }
    ...
    public Bitmap clip() {
        ...
        draw(canvas);
        float scale = (float) IMAGE_WIDTH / mFocusWidth;
        Matrix matrix = new Matrix();
        matrix.postScale(scale, scale); //���Ϳ�Ŵ���С�ı���
        //    	 Bitmap.createBitmap(bitmap, x, y, width, height, m, true);
        //    	return Bitmap.createBitmap(bitmap, (getWidth() - mFocusWidth) / 2, (getHeight() - mFocusHeight) / 2, mFocusWidth, mFocusHeight);
        return Bitmap.createBitmap(bitmap, (getWidth() - mFocusWidth) / 2, (getHeight() - mFocusHeight) / 2, mFocusWidth,
                mFocusHeight, matrix, true);
    }
}
