 ...
 */
public class CoverClipImageView extends ImageView implements View.OnTouchListener, ViewTreeObserver.OnGlobalLayoutListener {

	private static final int BORDERDISTANCE = ClipView.BORDERDISTANCE;
	private static final int IMAGE_WIDTH = ClipView.IMAGE_WIDTH;
	...
	public CoverClipImageView(Context context) {
		this(context, null);
	...
	public CoverClipImageView(Context context, AttributeSet attr) {
		this(context, attr, 0);
		this.mContext = context;
	}

	public CoverClipImageView(Context context, AttributeSet attr, int defStyle) {
	...
	private void configPosition() {
		...
		Float hf = (float) mFocusWidth * ClipView.HEIGHT_GRAVITY / ClipView.WIDTH_GRAVITY;
	...
	private class AnimatedZoomRunnable implements Runnable {
		...
		public void run() {
			...
			if (((deltaScale > 1f) && (currentScale < targetZoom)) || ((deltaScale < 1f) && (targetZoom < currentScale))) {
				...
				// again
				postOnAnimation(CoverClipImageView.this, this);
	...
	public Bitmap clip() {
		...
		float scale = (float) width / mFocusWidth;
		...
		return Bitmap.createBitmap(bitmap, (getWidth() - mFocusWidth) / 2, (getHeight() - mFocusHeight) / 2, mFocusWidth,
				(int) (mFocusHeight * 6.6f / 10f), matrix, true);
	}
}
