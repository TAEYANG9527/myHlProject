 ...
 * @功能说明 联系人列表适配器带字母
 */
public class PinnedContactAdapter extends ArrayAdapter<ContactItem> implements PinnedSectionListAdapter {
    ...
    public PinnedContactAdapter(Context context, int resource, int textViewResourceId) {
    ...
    // 列表添加项
    protected void onSectionAdded(ContactItem section, int sectionPosition) {
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            viewHolder.company_txt = (TextView) view.findViewById(R.id.company_txt);// 联系人所在的公司
        ...
        if (item.type == ContactItem.SECTION) {
            ...
            if (item.id.equals(IS_NEWFRIEND_ID)) {
                ...
                if (position < getCount() - 1) {
                    ContactItem tmpItem = getItem(position + 1);
            ...
            //数据加载
            Contact ct = item.getContact();
            if (null != ct) {
                /**0: 好友;1:手机通讯录;2:名片;3:手机通讯录和名片中非好友的Renhe会员**/
                int type = ct.getProfileSourceType();
                ...
                int colorIndex = ct.getColorIndex();
                ...
                if (!TextUtils.isEmpty(userCompany)) {
                    viewHolder.company_txt.setVisibility(View.VISIBLE);
                    viewHolder.company_txt.setText(userCompany);
                } else {
                    viewHolder.company_txt.setVisibility(View.GONE);
                }
