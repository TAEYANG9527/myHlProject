...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager() {
        ...
        mOkHttpClient.setConnectTimeout(Constants.ConfigureTimeouts.CONNECT_TIME_OUT, TimeUnit.SECONDS);
        mOkHttpClient.setWriteTimeout(Constants.ConfigureTimeouts.GET_REQUEST_TIME_OUT, TimeUnit.SECONDS);
        mOkHttpClient.setReadTimeout(Constants.ConfigureTimeouts.REQUEST_TIME_OUT, TimeUnit.SECONDS);
        //cookie enabled
    ...
    public static String postAsyn(String url, Map<String, Object> params, Class<?> clazz) throws Exception {
        ...
        if (null != params) {
            ...
            if (Constants.DEBUG_MODE) {
                Log.d("OKhttp***" + url + "", logBuffer + "");
                Log.d("OKhttp***" + url, url + "?" + paras.toString());
            }
     ...
     * @param callback
     */
    public static void postPhotoAsyn(String url, String fileKey, File file, Map<String, Object> params, Class<?> clazz, final ResultCallback callback, Object tag) {
        // 放置参数
        if (null != params) {
            StringBuffer logBuffer = new StringBuffer();
            StringBuilder paras = new StringBuilder("");// 打印字符请求
            ...
            if (Constants.DEBUG_MODE) {
                Logger.w("上传图片" + url + "?" + paras);
            }
        }
        Param[] paramsArr = map2Params(params);
        getInstance()._getUploadDelegate().postAsyn(url, fileKey, file, paramsArr, clazz, callback, tag);
    }
    ...
    private void deliveryResult(ResultCallback callback, final Request request, final Class<?> clazz) {
        ...
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(final Request request, final IOException e) {
                ...
                if (Constants.DEBUG_MODE) {
                    Logger.e(request.urlString() + "——response: 失败");
                    e.printStackTrace();
                }
            }
            ...
            public void onResponse(final Response response) {
                try {
                    ...
                    if (Constants.DEBUG_MODE) {
                        Logger.e(request.urlString() + "——response: " + string + "");
                    }
...
//			mType = getSuperclassTypeParameter(getClass());
    ...
    public class UploadDelegate {
         ...
         * 异步基于post的文件上传:主方法
         */
        public void postAsyn(String url, String[] fileKeys, File[] files, Param[] params, Class<?> clazz, ResultCallback callback, Object tag) {
            Request request = buildMultipartFormRequest(url, files, fileKeys, params, tag);
            deliveryResult(callback, request, clazz);
        }
         ...
         */
        public void postAsyn(String url, String fileKey, File file, Class<?> clazz, ResultCallback callback, Object tag) throws IOException {
            postAsyn(url, new String[]{fileKey}, new File[]{file}, null, clazz, callback, tag);
        }

        /**
         * 异步基于post的文件上传，单文件且携带其他form参数上传
         */
        public void postAsyn(String url, String fileKey, File file, Param[] params, Class<?> clazz, ResultCallback callback, Object tag) {
            postAsyn(url, new String[]{fileKey}, new File[]{file}, params, clazz, callback, tag);
        }
         ...
         * @return
         */
        public Response buildUploadImageRequest(String url, String path) throws IOException {
            MediaType MEDIA_TYPE_PNG = MediaType.parse("image/png");
            //创建OkHttpClient实例

            MultipartBuilder builder = new MultipartBuilder().type(MultipartBuilder.FORM);

            //遍历path中图片绝对路径到builder，并约定key如“image”作为后台接受多张图片的key
            builder.addFormDataPart("image", null, RequestBody.create(MEDIA_TYPE_PNG, new File(path)));
