...
public class LoginIMErrorActivity extends BaseActivity implements OnClickListener {
    ...
    protected void findView() {
        ...
        materialDialogsUtil.getBuilder("提示", content, "重新登录", "继续使用").callback(new MaterialDialog.ButtonCallback() {
            @Override
            public void onPositive(MaterialDialog dialog) {
                new LogOutTask(LoginIMErrorActivity.this).executeOnExecutor(Executors.newCachedThreadPool());
                // 删除信鸽推送设置
                delMyJPush(RenheApplication.getInstance().getUserInfo());
                isClick = true;
                new LogoutUtil(LoginIMErrorActivity.this, null).closeLogin(false);
                finish();
            }
