...
public class AisenReplyTextView extends TextView {
    ...
    private MessageBoards.ReplyList replyList;

    public AisenReplyTextView(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context);
    }

    public AisenReplyTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public int getEmojiSize() {
        if (null != BaseActivity.getRunningActivity())
            return BaseActivity.getRunningActivity().getResources().getDimensionPixelSize(R.dimen.S7);
        else
            return RenheApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.S7);
    }

    public MessageBoards.ReplyList getReplyList() {
        return replyList;
    }

    public void setReplyList(MessageBoards.ReplyList replyList) {
    ...
    public void setContent(String text) {
        ...
        if (spannableString != null) {
            ...
            loadReplyView(replyList, spannableString);
    ...
    class EmotionTask extends WorkTask<Void, SpannableString, Boolean> {
        ...
        public Boolean workInBackground(Void... params) throws TaskException {
            ...
            if (TextUtils.isEmpty(textView.getText()))
                return false;
            SpannableString spannableString = SpannableString.valueOf(textView.getText().toString());
            ...
            while (localMatcher.find()) {
                ...
                if (data == null)
                    continue;
                MyBitmap mb = BitmapLoader.getInstance().getImageCache().getSmallBitmapFromMemCache(key, null);
                Bitmap b = null;
                if (mb != null) {
                    b = mb.getBitmap();
                } else {
...
//                    int size = BaseActivity.getRunningActivity().getResources().getDimensionPixelSize(R.dimen.emotion_size);
                    ...
                    // 添加到内存中
                    BitmapLoader.getInstance().getImageCache().addSmallBitmapToMemCache(key, null, new MyBitmap(b, key));
                }
...
//            Linkify.addLinks(spannableString, dd, scheme);
            ...
            for (URLSpan urlSpan : urlSpans) {
                weiboSpan = new CustomURLSpan(urlSpan.getURL(), null, false);
        ...
        protected void onProgressUpdate(SpannableString... values) {
            ...
            try {
                if (values != null && values.length > 0) {
                    SpannableString spannableString = values[0];
                    loadReplyView(replyList, spannableString);
...
//                    }
    ...
    private void loadReplyView(MessageBoards.ReplyList replyList, SpannableString replySpannableString) {
        if (null == BaseActivity.getRunningActivity())
            return;
        if (null != replyList) {
            if (!TextUtils.isEmpty(replyList.getSenderName())) {
                ...
                replySpannableString.setSpan(
                        new ForegroundColorSpan(BaseActivity.getRunningActivity().getResources().getColor(R.color.reply_name_color)), 0,
                        senderName.length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                replySpannableString.setSpan(new RenmaiQuanUtils.MessageMemberSpanClick(senderSid), 0,
                        senderName.length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                if (!TextUtils.isEmpty(reSenderName)) {
                    replySpannableString.setSpan(
                            new ForegroundColorSpan(BaseActivity.getRunningActivity().getResources().getColor(R.color.reply_name_color)),
                            senderName.length() + 2,
                            senderName.length() + 2
                                    + reSenderName.length(),
                            Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
                    replySpannableString.setSpan(new RenmaiQuanUtils.MessageMemberSpanClick(reSenderSid),
                            senderName.length() + 2,
                            senderName.length() + 2
                                    + reSenderName.length(),
                            Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
