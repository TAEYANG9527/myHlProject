...
public class EmotionsDB {
    ...
    // 创建表情库
    static {
        String path = Constants.CACHE_PATH.EMOTION_DB_PATH;
...
//                        Set<Object> keyArray = properties.keySet();
                        String[] keyArray = RenheApplication.getInstance().getResources().getStringArray(R.array.face_zh);
                        // 开启事务
...
//		key = converter.convert(key);
    ...
    public static Emotions getEmotions(String type) {
        ...
        String query = type.indexOf("hl_") == -1 ? "unlike" : "like";
        query = "like";
