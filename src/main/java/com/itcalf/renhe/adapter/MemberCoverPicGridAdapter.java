...
public class MemberCoverPicGridAdapter extends BaseAdapter {
	private Context context;
	private MemberCover[] picLists;
	...
	public MemberCoverPicGridAdapter(Context context, MemberCover[] picLists) {
		this.context = context;
		this.picLists = picLists;
		this.imageLoader = ImageLoader.getInstance();
		this.options = new DisplayImageOptions.Builder().showImageOnLoading(DEFAULT_IMAGE).showImageForEmptyUri(DEFAULT_IMAGE)
		...
		DEFAULT_IMAGE = R.drawable.member_cover_pic_default_bcg;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (position < picLists.length) {
			String url = picLists[position].getMinCover();
			if (!TextUtils.isEmpty(url)) {
				try {
					imageLoader.displayImage(url, holder.image, options, CacheManager.animateFirstDisplayListener);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
